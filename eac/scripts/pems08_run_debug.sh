#!/bin/bash
# ===========================================================================
# PEMS08 (xxltrafficdata) 冒烟测试脚本 (pems08_run.sh 的 Debug 版)
# ---------------------------------------------------------------------------
...
#   * epoch=1                          (训练循环只跑 1 轮)
#   * 每个方法的 JSON 被临时克隆到 /tmp/eac_debug_conf/PEMS08/，只改少数字段
#   * 不触碰你原来的 conf/PEMS08/*.json
#
# 数据路径: ../xxltrafficdata/preprocessed/pems08/{RawData,FastData,graph}/
# 首年: 2001 (= begin_year)
# 日志目录: log/PEMS08/ (debug 产物 logname 加 _debug 后缀)
# ---------------------------------------------------------------------------
# 用法：
#   cd eac/
#   bash scripts/pems08_run_debug.sh
# ===========================================================================
...
# ---------------------------------------------------------------------------
# 后台模式: 用 `NOHUP=1 bash scripts/pems08_run_debug.sh` 触发
# 自动 nohup 后台跑, 日志落到 eac/run_logs/pems08_debug_<时间戳>.log
# ---------------------------------------------------------------------------
if [[ "${NOHUP:-0}" == "1" && -z "${EAC_BG:-}" ]]; then
    mkdir -p run_logs
    LOG_FILE="run_logs/pems08_debug_$(date +%Y%m%d_%H%M%S).log"
...
DEBUG_CONF_DIR="/tmp/eac_debug_conf/PEMS08"
...
# ============================================================================
echo "==================== [1/8] Retrain ===================="
python main.py --conf "$DEBUG_CONF_DIR/retrain_st_pems08.json" --gpuid "$GPU" --seed "$SEED_MAIN"
...
src_dir="${LOG_ROOT}/retrain_st_pems08_${DEBUG_LOG_SUFFIX}-${SEED_MAIN}/${FIRST_YEAR}"
dst_dir="${LOG_ROOT}/pretrain_st_pems08_${DEBUG_LOG_SUFFIX}-${SEED_MAIN}/${FIRST_YEAR}"
...
python main.py --conf "$DEBUG_CONF_DIR/pretrain_st_pems08.json" \
    --load_first_year 1 \
...
# ============================================================================
echo "==================== [4/8] Online-NN ===================="
python main.py --conf "$DEBUG_CONF_DIR/oneline_st_nn_pems08.json" --gpuid "$GPU" --seed "$SEED_MAIN"
...
# ============================================================================
echo "==================== [5/8] Online-AN ===================="
python main.py --conf "$DEBUG_CONF_DIR/oneline_st_an_pems08.json" --gpuid "$GPU" --seed "$SEED_ALT"
