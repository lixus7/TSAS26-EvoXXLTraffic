#!/bin/bash
# ===========================================================================
# PEMS04 (xxltrafficdata) 全流程重训脚本 (自动化版)
# ---------------------------------------------------------------------------
# 数据路径: ../xxltrafficdata/preprocessed/pems04/{RawData,FastData,graph}/
# 年份范围: 2002 ~ 2025  (PEMS04 的 conf/*.json 全部 begin_year=2002)
# 日志目录: log/PEMS04/
# ---------------------------------------------------------------------------
# 流水线 (同 pems05_run.sh，首年 2002, conf 改为 conf/PEMS04/*):
# 1. Retrain  : 每年从零训练 (2002~2025)，作为 Oracle 上界 & pretrain 的基座
# 2. AutoLink : 把 retrain 的 2002 权重软链到 pretrain_st_pems04-<seed>/2002/
# 3. Pretrain : 只用 2002 权重直接推理后续年份 (下界)
# 4~6.        : online-nn / online-an / TrafficStream 基线
# 7. PatchSTKEC: 自动把 conf/PEMS04/stkec.json 里的 influence_path 占位替换成真实目录
# 8. STKEC / 9. EAC : 继续跑剩余方法
# ---------------------------------------------------------------------------
# 用法：
#   cd eac/
#   bash scripts/pems04_run.sh
#
# 可选环境变量：
#   NOHUP=1            后台跑 + 写日志
#   SKIP_RETRAIN=1     跳过步骤 1 (retrain 已完成的情况下，从 AutoLink 继续跑)
# ===========================================================================
...
# ---------------------------------------------------------------------------
# 后台模式: 用 `NOHUP=1 bash scripts/pems04_run.sh` 触发
# 自动 nohup 后台跑, 日志落到 eac/run_logs/pems04_<时间戳>.log
# ---------------------------------------------------------------------------
if [[ "${NOHUP:-0}" == "1" && -z "${EAC_BG:-}" ]]; then
    mkdir -p run_logs
    LOG_FILE="run_logs/pems04_$(date +%Y%m%d_%H%M%S).log"
...
FIRST_YEAR=2002    # PEMS04 的 conf/*.json 全部 begin_year=2002，必须与之保持一致
LOG_ROOT="log/PEMS04"
...
if [[ "${SKIP_RETRAIN:-0}" == "1" ]]; then
    echo "==================== [1/8] Retrain (SKIPPED via SKIP_RETRAIN=1) ===================="
else
    echo "==================== [1/8] Retrain ===================="
    for seed in "${SEEDS_MAIN[@]}"; do
        python main.py --conf conf/PEMS04/retrain_st_pems04.json --gpuid $GPU_MAIN --seed "$seed"
    done
fi
...
for seed in "${SEEDS_MAIN[@]}"; do
    src_dir="${LOG_ROOT}/retrain_st_pems04-${seed}/${FIRST_YEAR}"
    dst_dir="${LOG_ROOT}/pretrain_st_pems04-${seed}/${FIRST_YEAR}"
...
echo "==================== [3/8] Pretrain ===================="
for seed in "${SEEDS_MAIN[@]}"; do
    first_year_pkl=$(ls ${LOG_ROOT}/retrain_st_pems04-${seed}/${FIRST_YEAR}/*.pkl 2>/dev/null | head -n 1)
    if [ -z "$first_year_pkl" ]; then
        echo "  [skip] seed=$seed 缺少 retrain ${FIRST_YEAR} 权重"; continue
    fi
    python main.py --conf conf/PEMS04/pretrain_st_pems04.json \
        --load_first_year 1 \
...
echo "==================== [4/8] Online-NN ===================="
for seed in "${SEEDS_MAIN[@]}"; do
    python main.py --conf conf/PEMS04/oneline_st_nn_pems04.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_ALT[@]}"; do
    python main.py --conf conf/PEMS04/oneline_st_an_pems04.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_MAIN[@]}"; do
    python main.py --conf conf/PEMS04/trafficstream.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_STKEC[@]}"; do
    python stkec_main.py --conf conf/PEMS04/stkec.json --gpuid $GPU_STKEC --seed "$seed"
done
...
# 8. EAC (本论文主方法)
# ============================================================================
# 注意：conf/PEMS04/eac.json 默认 "train": 1, "auto_test": 0 —— 即“从零训练 EAC”。
#       如果你要**复用已有权重做评测**，请把 JSON 改成 "train": 0, "auto_test": 1，
#       并确保 ${LOG_ROOT}/eac-<seed>/${FIRST_YEAR}/*.pkl 存在。
echo "==================== [8/8] EAC ===================="
for seed in "${SEEDS_ALT[@]}"; do
    python main.py --conf conf/PEMS04/eac.json --gpuid $GPU_EAC --seed "$seed"
done
