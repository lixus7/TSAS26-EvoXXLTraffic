#!/bin/bash
# ===========================================================================
# PEMS05 (xxltrafficdata) 冒烟测试脚本 (pems05_run.sh 的 Debug 版)
# ---------------------------------------------------------------------------
...
#   * epoch=1                          (训练循环只跑 1 轮)
#   * 每个方法的 JSON 被临时克隆到 /tmp/eac_debug_conf/PEMS05/，只改少数字段
#   * 不触碰你原来的 conf/PEMS05/*.json
#
# 数据路径: ../xxltrafficdata/preprocessed/pems05/{RawData,FastData,graph}/
# 首年: 2005 (= begin_year)
# 日志目录: log/PEMS05/ (debug 产物 logname 加 _debug 后缀)
# ---------------------------------------------------------------------------
# 用法：
#   cd eac/
#   bash scripts/pems05_run_debug.sh
# ===========================================================================
...
# ---------------------------------------------------------------------------
# 后台模式: 用 `NOHUP=1 bash scripts/pems05_run_debug.sh` 触发
# 自动 nohup 后台跑, 日志落到 eac/run_logs/pems05_debug_<时间戳>.log
# ---------------------------------------------------------------------------
if [[ "${NOHUP:-0}" == "1" && -z "${EAC_BG:-}" ]]; then
    mkdir -p run_logs
    LOG_FILE="run_logs/pems05_debug_$(date +%Y%m%d_%H%M%S).log"
...
DEBUG_CONF_DIR="/tmp/eac_debug_conf/PEMS05"
...
# ============================================================================
echo "==================== [1/8] Retrain ===================="
python main.py --conf "$DEBUG_CONF_DIR/retrain_st_pems05.json" --gpuid "$GPU" --seed "$SEED_MAIN"
...
src_dir="${LOG_ROOT}/retrain_st_pems05_${DEBUG_LOG_SUFFIX}-${SEED_MAIN}/${FIRST_YEAR}"
dst_dir="${LOG_ROOT}/pretrain_st_pems05_${DEBUG_LOG_SUFFIX}-${SEED_MAIN}/${FIRST_YEAR}"
...
python main.py --conf "$DEBUG_CONF_DIR/pretrain_st_pems05.json" \
    --load_first_year 1 \
...
# ============================================================================
echo "==================== [4/8] Online-NN ===================="
python main.py --conf "$DEBUG_CONF_DIR/oneline_st_nn_pems05.json" --gpuid "$GPU" --seed "$SEED_MAIN"
...
# ============================================================================
echo "==================== [5/8] Online-AN ===================="
python main.py --conf "$DEBUG_CONF_DIR/oneline_st_an_pems05.json" --gpuid "$GPU" --seed "$SEED_ALT"
