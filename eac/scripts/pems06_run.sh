#!/bin/bash
# ===========================================================================
# PEMS06 (xxltrafficdata) 全流程重训脚本 (自动化版)
# ---------------------------------------------------------------------------
# 数据路径: ../xxltrafficdata/preprocessed/pems06/{RawData,FastData,graph}/
# 年份范围: 2005 ~ 2025
# 日志目录: log/PEMS06/
# ---------------------------------------------------------------------------
# 流水线 (同 pems05_run.sh，首年 2005, conf 改为 conf/PEMS06/*):
# 1. Retrain  : 每年从零训练 (2005~2025)，作为 Oracle 上界 & pretrain 的基座
# 2. AutoLink : 把 retrain 的 2005 权重软链到 pretrain_st_pems06-<seed>/2005/
# 3. Pretrain : 只用 2005 权重直接推理后续年份 (下界)
# 4~6.        : online-nn / online-an / TrafficStream 基线
# 7. PatchSTKEC: 自动把 conf/PEMS06/stkec.json 里的 influence_path 占位替换成真实目录
# 8. STKEC / 9. EAC : 继续跑剩余方法
# ---------------------------------------------------------------------------
# 用法：
#   cd eac/
#   bash scripts/pems06_run.sh
# ===========================================================================
...
# ---------------------------------------------------------------------------
# 后台模式: 用 `NOHUP=1 bash scripts/pems06_run.sh` 触发
# 自动 nohup 后台跑, 日志落到 eac/run_logs/pems06_<时间戳>.log
# ---------------------------------------------------------------------------
if [[ "${NOHUP:-0}" == "1" && -z "${EAC_BG:-}" ]]; then
    mkdir -p run_logs
    LOG_FILE="run_logs/pems06_$(date +%Y%m%d_%H%M%S).log"
...
for seed in "${SEEDS_MAIN[@]}"; do
    python main.py --conf conf/PEMS06/retrain_st_pems06.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_MAIN[@]}"; do
    src_dir="${LOG_ROOT}/retrain_st_pems06-${seed}/${FIRST_YEAR}"
    dst_dir="${LOG_ROOT}/pretrain_st_pems06-${seed}/${FIRST_YEAR}"
...
echo "==================== [3/8] Pretrain ===================="
for seed in "${SEEDS_MAIN[@]}"; do
    first_year_pkl=$(ls ${LOG_ROOT}/retrain_st_pems06-${seed}/${FIRST_YEAR}/*.pkl 2>/dev/null | head -n 1)
    if [ -z "$first_year_pkl" ]; then
        echo "  [skip] seed=$seed 缺少 retrain ${FIRST_YEAR} 权重"; continue
    fi
    python main.py --conf conf/PEMS06/pretrain_st_pems06.json \
        --load_first_year 1 \
...
echo "==================== [4/8] Online-NN ===================="
for seed in "${SEEDS_MAIN[@]}"; do
    python main.py --conf conf/PEMS06/oneline_st_nn_pems06.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_ALT[@]}"; do
    python main.py --conf conf/PEMS06/oneline_st_an_pems06.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_MAIN[@]}"; do
    python main.py --conf conf/PEMS06/trafficstream.json --gpuid $GPU_MAIN --seed "$seed"
done
...
for seed in "${SEEDS_STKEC[@]}"; do
    python stkec_main.py --conf conf/PEMS06/stkec.json --gpuid $GPU_STKEC --seed "$seed"
done
...
# 8. EAC (本论文主方法)
# ============================================================================
# 注意：conf/PEMS06/eac.json 默认 "train": 1, "auto_test": 0 —— 即“从零训练 EAC”。
#       如果你要**复用已有权重做评测**，请把 JSON 改成 "train": 0, "auto_test": 1，
#       并确保 ${LOG_ROOT}/eac-<seed>/${FIRST_YEAR}/*.pkl 存在。
echo "==================== [8/8] EAC ===================="
for seed in "${SEEDS_ALT[@]}"; do
    python main.py --conf conf/PEMS06/eac.json --gpuid $GPU_EAC --seed "$seed"
done
